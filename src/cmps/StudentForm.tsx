...
export function StudentForm({ isOpen, onClose, student }: StudentFormProps) {
  const { saveStudent, isLoading, error, clearError } = useStudentStore()
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      ...
      try {
        const teacherData = await teacherService.getTeachers({ isActive: true })
    ...
    if (student?._id) {
      setFormData({
        ...
        academicInfo: {
          ...
          tests: {
            stageTest: {
              status: student.academicInfo?.tests?.stageTest?.status || 'לא נבחן',
              lastTestDate: student.academicInfo?.tests?.stageTest?.lastTestDate,
              nextTestDate: student.academicInfo?.tests?.stageTest?.nextTestDate,
              notes: student.academicInfo?.tests?.stageTest?.notes || '',
            },
            technicalTest: {
              status: student.academicInfo?.tests?.technicalTest?.status || 'לא נבחן',
              lastTestDate: student.academicInfo?.tests?.technicalTest?.lastTestDate,
              nextTestDate: student.academicInfo?.tests?.technicalTest?.nextTestDate,
  ...
  const handleLessonScheduleChange = (e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.personalInfo?.phone && !/^05\d{8}$/.test(formData.personalInfo.phone)) {
      newErrors['personalInfo.phone'] = 'מספר טלפון לא תקין (צריך להתחיל ב-05 ולכלול 10 ספרות)'
    }
    
    if (formData.personalInfo?.parentPhone && !/^05\d{8}$/.test(formData.personalInfo.parentPhone)) {
      newErrors['personalInfo.parentPhone'] = 'מספר טלפון הורה לא תקין (צריך להתחיל ב-05 ולכלול 10 ספרות)'
    }
    
    if (formData.personalInfo?.parentEmail && 
        !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.personalInfo.parentEmail)) {
      newErrors['personalInfo.parentEmail'] = 'כתובת אימייל הורה לא תקינה'
    }
    
    if (formData.personalInfo?.studentEmail && 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { teacherId, lessonDay, lessonTime, lessonDuration, ...restFormData } = formData
      ...
      const dataToSubmit = {
        ...
        personalInfo: {
          ...
          parentEmail: formData.personalInfo.parentEmail || "parent@example.com",
          studentEmail: formData.personalInfo.studentEmail || "student@example.com"
        },
        enrollments: {
      ...
      if (currentSchoolYear && !dataToSubmit.enrollments?.schoolYears?.some(
        (sy: { schoolYearId: string }) => sy.schoolYearId === currentSchoolYear._id
      )) {
      ...
      const savedStudent = await saveStudent(dataToSubmit)
            ...
            <div className="form-row full-width">
              <div className="form-group">
                ...
                {errors['personalInfo.studentEmail'] && (
                  <div className="form-error">{errors['personalInfo.studentEmail']}</div>
                )}
              </div>
            </div>
            ...
            <div className="form-row two-columns">
              ...
              <div className="form-group">
                ...
                {errors['personalInfo.parentPhone'] && (
                  <div className="form-error">{errors['personalInfo.parentPhone']}</div>
                )}
              </div>
            </div>
            ...
            <div className="form-row full-width">
              <div className="form-group">
                ...
                {errors['personalInfo.parentEmail'] && (
                  <div className="form-error">{errors['personalInfo.parentEmail']}</div>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="form-row three-columns">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  value={formData.academicInfo?.tests?.technicalTest?.status || 'לא נבחן'}
                  onChange={(e) => handleTestChange('technicalTest', 'status', e.target.value)}
                >
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  value={formData.academicInfo?.tests?.stageTest?.status || 'לא נבחן'}
                  onChange={(e) => handleTestChange('stageTest', 'status', e.target.value)}
                >
                  {TEST_STATUSES.map(status => (
                    <option key={status} value={status}>
                      {status}
                ...
                >
                  ...
                  {teachers.map(teacher => (
                    <option key={teacher._id} value={teacher._id}>
                      {teacher.personalInfo.fullName} {teacher.professionalInfo?.instrument ? `(${teacher.professionalInfo.instrument})` : ''}
                    </option>
                  ))}
                </select>
